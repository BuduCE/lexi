...
const address = ref('');
const phone = ref('');
const total = ref(formatAmount(totalPrice));

const sendMail = async () => {

    if (!email.value || !name.value || !address.value || !phone.value || !total.value) return;
...
<template>
    ...
    <form ref="formEl" @submit.prevent="sendMail">
        ...
        <div class="total">
            <h4>Total</h4>
            <textarea v-model="total" id="total" name="total" placeholder="₦{{ formatAmount(totalPrice) }}>₦{{ formatAmount(totalPrice) }}</textarea>
        </div>
