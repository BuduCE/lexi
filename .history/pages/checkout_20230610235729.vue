...
const address = ref('');
const phone = ref('');
const cart = ref(cartproduct.name '- ₦'formatAmount(cartproduct.price * cartproduct.count));
const total = ref(totalPrice);

const sendMail = async () => {

    if (!email.value || !name.value || !address.value || !phone.value) return;
    ...
    try {
        ...
        console.log('Service_ID', runtimeConfig.public.serviceId)
    } catch (error) {
        btnText.value = 'Fail!';
        setTimeout(() => {
            btnText.value = 'Send';
...
<template>
    ...
    <form ref="formEl" @submit.prevent="sendMail">
        ...
        <div class="billing_info containers">
            ...
            <textarea v-model="cart" name="cart" id="" cols="30" rows="5" readonly="readonly" v-for="cartproduct in shoppingCart" :key="cartproduct.id">
                {{ cartproduct.name }} - ₦{{ formatAmount(cartproduct.price * cartproduct.count) }}
            </textarea>

            <textarea v-model="total" name="total" id="" cols="30" rows="5" readonly="readonly">
                {{ totalPrice }}
            </textarea>
        </div>
