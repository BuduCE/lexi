...
const imageList = [{src:product.img}, product.imgT,product.imgTh
];

const selected = ref(imageList[0])

const switchImages = (index) => {
    selected.value = imageList.value[index];
...
<template>
    ...
    <section class="product-pg-info">
        <div class="product-pg-img">
            <img :src="selected" alt="" class="big-pg-img" id="mainImg">

            <div class="small-img-grp">
                <div class="small-img" v-for="(image, index) in imageList.values" >
                    <img :src="`${image}`" alt="" class="small-pg-img" :key="index" @click="switchImages(index)">
                </div>
            </div>
        </div>
