...
const addToCart = (cartProductID) => {
    shoppingcart.value.push(cartProductID) 
    console.log(shoppingcart.value)
    const exis = JSON.parse(localStorage.getItem('shoppingcart') || "[]")
...
const getImage = async (imgName) => {
    ...
    const module = await import(/* @vite-ignore */ `~/assets/storeList/${imgName}.png`)
    const any = await module.default.replace(/^\/@fs/, '')
    return any
}
</script>
    ...
    <section>
        ...
        <section class="store_section">
            <div class="items slide_pane" id="slides">
                ...
                <NuxtLink v-for="product in products" :key="product.id"  id="productOne" class="good">
                    <img :src="`~/assets/storeList/${product.img}.png`" alt="" class="img_class">
