...
<template>
    ...
    <form ref="formEl" @submit.prevent="sendMail">
        ...
        <div class="billing_info containers">
            ...
            <textarea v-model="phone" name="cart" id="" cols="30" rows="5" readonly="readonly" v-for="cartproduct in shoppingCart" :key="cartproduct.id">
                {{ cartproduct.name }} - ₦{{ formatAmount(cartproduct.price * cartproduct.count) }}
            </textarea>

            <textarea name="total" id="" cols="30" rows="5" readonly="readonly">
                {{ totalPrice }}
            </textarea>
        </div>
